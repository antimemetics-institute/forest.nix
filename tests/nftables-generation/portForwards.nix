...
let
  ...
  vm = { ipv4, ipv6, portForwards ? [] }: { inherit ipv4 ipv6 portForwards; };
  ...
  testCasesV4 = {
    ...
    interfaceOnlyDefaultsBoth = {
      ...
      input = {
        dev = vm (dev // { portForwards = [ (pf { interface = "tailscale0"; }) ]; });
      };
    ...
    interfaceWithV4Sentinel = {
      ...
      input = {
        dev = vm (dev // { portForwards = [ (pf { interface = "tailscale0"; bindAddress = "0.0.0.0"; }) ]; });
      };
      expected = ''            iifname "tailscale0" tcp dport 22 dnat to 192.168.69.10:22'';
    };
    ...
    interfaceWithV6SentinelOnly = {
      ...
      input = {
        dev = vm (dev // { portForwards = [ (pf { interface = "tailscale0"; bindAddress = "::"; }) ]; });
      };
      expected = "";
    };
    ...
    specificV4BindAddress = {
      input = {
        dev = vm (dev // { portForwards = [ (pf { port = 80; hostPort = 8080; protocol = "tcp"; bindAddress = "203.0.113.5"; }) ]; });
      };
    ...
    interfaceAndSpecificV4 = {
      input = {
        dev = vm (dev // { portForwards = [ (pf { port = 22; interface = "wg0"; bindAddress = "10.0.0.1"; }) ]; });
      };
    ...
    mixedListBindAddress = {
      ...
      input = {
        dev = vm (dev // { portForwards = [ (pf { interface = "tailscale0"; bindAddress = [ "100.64.0.1" "fd7a:115c::1" ]; }) ]; });
      };
    ...
    protocolBothExpands = {
      input = {
        dev = vm (dev // { portForwards = [ (pf { protocol = "both"; interface = "tailscale0"; }) ]; });
      };
      expected =
    ...
    multipleVms = {
      input = {
        db  = vm (db  // { portForwards = [ (pf { port = 5432; protocol = "tcp"; interface = "wg0"; }) ]; });
        dev = vm (dev // { portForwards = [ (pf { port = 22; protocol = "tcp"; interface = "tailscale0"; }) ]; });
      };
      expected =
  ...
  testCasesV6 = {
    ...
    specificV6BindAddress = {
      input = {
        dev = vm (dev // { portForwards = [ (pf { port = 80; hostPort = 8080; bindAddress = "fd7a:115c::1"; }) ]; });
      };
